...
// sidebar_footer single_winner slots at order 50 (the upstream footer
// registers at 100; the title slot's upstream rendering is its fallback
// child). The title gains the bronze meander rule under it, and the footer's
// "• OpenCode" word-pair becomes the brand line "◆ ATHENA CODE". Both carry
// everything the upstream renderings showed: session id on non-latest
// channels, workspace label, share url, the getting-started promo, and the
// directory:branch path (mirrored from routes/session/sidebar.tsx and
// feature-plugins/sidebar/footer.tsx so taking the slots loses nothing).
...
// Narrower than the home hero's rule so it fits the 42-column sidebar's
